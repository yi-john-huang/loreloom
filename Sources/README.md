...
After capture and review, treat Sources as append-only; corrections and later manual classifications use dated Amendments. Interpretations belong in `Knowledge/`, broad synthesis belongs in `Wiki/`, and copyrighted material should normally remain linked rather than copied into a public vault. See `docs/FRONTMATTER.md` for the exact enum, locator, boundary, and manual-migration contracts.
