...
5. Read each Source completely and record its `capture_mode` before extracting claims. Record unavailable attachments or external material instead of guessing.
...
10. When any input Source uses `capture_mode: unknown`, `paraphrased`, or `reference-only`, add a non-empty `## Evidence limitations` section to every affected draft. Include each applicable statement:
    ...
    Retain weaker-source limitations when stronger evidence is also cited.
...
List revalidated Source paths and capture modes, created, updated, skipped, duplicate, contradictory, and uncertain items; include every draft path and citation, conflicts and Evidence limitations, and the owner-only final claim/citation/limitation review and optional evergreen promotion. Every draft must have valid evidence links and a conservative confidence level.
